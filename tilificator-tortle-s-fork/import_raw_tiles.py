...
# TODO: Check formats against tile settings, and map 8x8 -> 8x16 appropriately
...
class RawSpritesDialog(QDialog):
    def __init__(self):
        super(RawSpritesDialog, self).__init__()
        self.fileName = None
        self.fileChooserButton = QPushButton('Browse')
        # Use form layout for formats/file button
        self.formLayout = QFormLayout()
...
class ImportRawTilesDialog(RawTilesDialog):
    def __init__(self):
        super(ImportRawTilesDialog, self).__init__()
...
class ExportRawTilesDialog(RawTilesDialog):
    def __init__(self):
        super(ExportRawTilesDialog, self).__init__()
...
class ExportSpriteSheetDialog(RawSpritesDialog):
    ### Opens a new dialog box that allows you to select a file to save to
    def __init__(self):
        super(ExportSpriteSheetDialog, self).__init__()
        self.setWindowTitle('Export spritesheet')
        self.fileChooserButton.setText('Save As File...')
        self.fileChooserButton.clicked.connect(self.saveFile)
    ...
    def saveFile(self, filechooserbutton):
        filename, fileType = QFileDialog.getSaveFileName(parent=self, caption='Save spritesheet file')
        if filename != '':
            self.filename = filename
            self.buttonOk.setEnabled(True)

    def write_spritesheet(self, listOfSprites, numSprites):
        writeSprites(self.filename, listOfSprites, numSprites)   #DEFINE WRITESPRITES
...
def writeSprites(filename, listOfSprites, numSprites):
    fileContents = "Output_generatedMetasprite:\n"
    with open(filename, "w") as f:
        for i in range(numSprites):
            fileContents += "DW sprite_" + str(i) + "\n"
        fileContents+="\n"
        i = 0
        for si in listOfSprites:
            fileContents += "sprite_" + str(i) + ":\nDB  $" + "{:02X}".format(len(si.tilification.tiles)*4) + "\n"
            for tile in si.tilification.tiles:
                fileContents += "DB $" + "{:02X}".format((tile.y - si.originY)%256) + ", $" + "{:02X}".format(tile.tileID) + ", %" + ("1" if tile.flipV else "0") + ("1" if tile.flipH else "0") + "00000" + ("0" if tile.colorDataHi else "1") + ", $" + "{:02X}".format((tile.x - si.originX)%256) + "\n"
            fileContents += "\n"
            i += 1
