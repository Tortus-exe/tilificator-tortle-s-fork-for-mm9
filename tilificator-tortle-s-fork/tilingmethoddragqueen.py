...
from common import makeNext1, expandedProduct, indexOfMinimum, combinationsIncreasing, mergeSortedLists
from spritetile import *

from tilingmethod import *
...
class TilingMethodDragQueen(TilingMethod):
    ...
    def makeFirstNonBlankPixelInColumTable(self, si):
        ...
        firstNonBlankPixelTable = [paddedHeight] * paddedWidth * paddedHeight
    ...
    def columnBlank(self, si, xStart, yStart, height):
        paddedWidth = si.width + 2 * (self.settings.tileWidth - 1)
        return si.firstNonBlankPixelInColumnTable[yStart * paddedWidth + xStart] >= height
    ...
    def findFirstNonBlankColumn(self, spriteImage, spriteTile, yStart=0, yEnd=None):
        ...
        if columnHeight < 0:
            columnHeight = 0
        paddedWidth = spriteImage.width + 2 * (self.settings.tileWidth - 1)
        return spriteImage.firstNonBlankColumnTable[columnHeight][(spriteTile.y + yStart + self.settings.tileHeight - 1) * paddedWidth + spriteTile.x + self.settings.tileWidth - 1]
    ...
    def findLastNonBlankColumn(self, spriteImage, spriteTile, yStart=0, yEnd=None, debug=False):
        ...
        if columnHeight < 0:
            columnHeight = 0
        paddedWidth = spriteImage.width + 2 * (self.settings.tileWidth - 1)
        return spriteImage.lastNonBlankColumnTable[columnHeight][(spriteTile.y + yStart + self.settings.tileHeight - 1) * paddedWidth + spriteTile.x + self.settings.tileWidth - 1]
